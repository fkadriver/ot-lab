...
#
# Pre-requisites:
#   - source .env.armis              (all ARMIS_* vars must be set)
#   - GRFICSv3 lab must be running   (./scripts/start-lab.sh grfics)
#   - sudo access
#
# Usage:
#   source .env.armis
...
# All collector credentials come from environment / .env.armis — no hardcoded values.
COLLECTOR_ID="${ARMIS_COLLECTOR_ID:?ARMIS_COLLECTOR_ID not set — check .env.armis}"
COLLECTOR_LICENSE="${ARMIS_COLLECTOR_LICENSE:?ARMIS_COLLECTOR_LICENSE not set — check .env.armis}"
ARMIS_HOSTNAME="${ARMIS_HOSTNAME:?ARMIS_HOSTNAME not set — check .env.armis}"
COLLECTOR_NAME="${ARMIS_COLLECTOR_NAME:-GRFICSv3 Lab Collector}"
...
install_qemu() {
  ...
  info "Installing qemu-kvm, qemu-utils, ovmf..."
  apt-get update -qq
  apt-get install -y qemu-kvm qemu-utils ovmf
  info "QEMU installed."
}
...
1. Open:  https://localhost:18443  (user: config / pass: Armis)
   (From Azure: SSH tunnel — ssh -L 18443:localhost:18443 <vm-ip>)
